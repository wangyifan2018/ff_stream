...
void* push_thread(void* ff_encoder)
{
    ...
    while(!quit_flag)
    {
        if(output_type!=VIDEO_LOCAL_FILE)
            gettimeofday(&curframe_start, NULL);
        ...
        if(q.empty())
        {
            ...
            if(output_type!=VIDEO_LOCAL_FILE)
            {
                ...
                int64_t start_time = curframe_start.tv_sec*1000*1000 + curframe_start.tv_usec;
                int64_t current_time = curframe_push.tv_sec*1000*1000 + curframe_push.tv_usec;
                int64_t cost = current_time - start_time;
                if(cost < frame_interval)
                    usleep(frame_interval - cost);
            }
        }
    }
    bm_dev_free(handle);
...
int main(int argc, char** argv)
{
    ...
    int frame_count         = atoi(argv[9]);
    int sophon_idx          = atoi(argv[11]);
    bm_handle_t handle;
    bm_dev_request(&handle, sophon_idx);
    bm_get_chipid(handle, &chip_id);

    // FILE* fp = fopen("/opt/sophon/sophon-sample-latest/bin/water.bin", "wb+");
    // if(!fp)
    // {
    //     int water_w = 117;
    //     int water_h = 79;
    //     int total = water_w*water_h;
    //     bm_device_mem_t* watermark = (bm_device_mem_t*)malloc(sizeof(bm_device_mem_t));
    ...
    encoder.openEnc(argv[2], encoder_name, enc_frame_rate, enc_width, enc_height, enc_pix_fmt, enc_bitrate, sophon_idx);
    ...
    timeval curframe_start, curframe_push;
    int frame_interval = 1*1000*1000 / enc_frame_rate;
    int output_type = encoder.get_output_type();
    ...
    while(push_count < frame_count)
    {
        ...
        if(ret < 0)
            break;
        AVFrameConvert(handle, frame, enc_frame, enc_height, enc_width, enc_pix_fmt, chip_id);
        // AVFrameConvert(handle, frame, enc_frame, enc_height, enc_width, enc_pix_fmt, chip_id, 0, 1, watermark);
        encoder.writeFrame(enc_frame);
        push_count++;
        av_frame_free(&frame);
        av_frame_free(&enc_frame);
...
static void usage(char *program_name)
{
    av_log(NULL, AV_LOG_ERROR, "\t read, decode, resize, encode, push stream or save local video file. \n");
    av_log(NULL, AV_LOG_ERROR, "Usage: \n\t%s [src_filename] [output_filename] [width] [height] [encode_pixel_format] [encoder_name] [frame_rate] [bitrate] [zero_copy] [sophon_idx] \n", program_name);
    ...
    av_log(NULL, AV_LOG_ERROR, "\t[optional: water mark file path]              water mark file path\n");
    av_log(NULL, AV_LOG_ERROR, "\tsuch as:  ./ff_stream_push input.mp4 rtsp://172.1.1.1:8554/test h264_bm NV12 640 640 25 1000 500 1 0\n");
}
