...
int avframe_to_bm_image(bm_handle_t &bm_handle,AVFrame &in, bm_image &out){
    ...
    if (in.channel_layout == 101) {/* COMPRESSED NV12 FORMAT */
        ...
        if(bm_image_alloc_dev_mem_heap_mask(out,USEING_MEM_HEAP2) != BM_SUCCESS){
...
int AVFrameConvert(bm_handle_t &bmHandle,AVFrame *inPic,AVFrame *outPic,int enc_frame_height,int enc_frame_width,int enc_pix_format,int chip_id, 
    ...
    if(bm_image_alloc_dev_mem_heap_mask(*bmImageout,USEING_MEM_HEAP2) != BM_SUCCESS){
        printf("bmcv allocate mem failed!!!");
    }
    ...
    if(bmOutFormat==FORMAT_YUV420P || chip_id==0x1686)
    {
        ...
        bm_image bgr_img;
        bm_image_create(bmHandle,enc_frame_height,enc_frame_width,FORMAT_BGR_PACKED, DATA_TYPE_EXT_1N_BYTE,&bgr_img);
        bmcv_rect_t crop_rect = {0, 0, inPic->width, inPic->height};
        ret = bmcv_image_vpp_convert(bmHandle, 1, *bmImagein, &bgr_img ,&crop_rect);
    	if(ret != BM_SUCCESS){
        	printf("convert error coed = %d\n",ret);
        	return -1;
    	}
        bmcv_image_storage_convert(bmHandle, 1, &bgr_img, bmImageout);
        bm_image_destroy(bgr_img);
    }
    // /* mosaic process */
    if (enable_mosaic){
    ...
    if (enable_watermark){
        ...
        if(bmImageout->width >= water_w + interval && bmImageout->height >= water_h + interval){
            ...
            bm_status_t ret = bmcv_image_watermark_repeat_superpose(bmHandle, *bmImageout, *watermark, font_num, 0, water_w, rect, color);
            free(rect);
        }
    }
